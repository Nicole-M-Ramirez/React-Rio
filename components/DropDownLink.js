    ...
    if (change === false){
        ...
        setSize(
            <TouchableOpacity style={[styles.buttonView,{height: DropSize,backgroundColor:Color}]} onPress={Resize}>
                ...
                <Text style={{width:dimensions.buttonWidth*1.99,color:'white', fontSize:normalize(11),top:20, left:dimensions.separator, marginTop:dimensions.bodyWidth*0.07}}>{contText}</Text>
                <Text style={{color: 'blue',width:dimensions.buttonWidth*1.99, fontSize:normalize(11),top:20, left:dimensions.separator, marginTop:dimensions.bodyWidth*0.07 }} onPress={() => Linking.openURL('https://www.selfinjury.bctr.cornell.edu/')}>{gs['queAutolecionCont2'][lang]}</Text>
                <Text style={{color: 'blue',width:dimensions.buttonWidth*1.99, fontSize:normalize(11),top:20, left:dimensions.separator, marginTop:dimensions.bodyWidth*0.07}} onPress={() => Linking.openURL('https://www.itriples.org/')}>{gs['queAutolecionCont3'][lang]}</Text>
            </TouchableOpacity>
        )
    }
    else{
